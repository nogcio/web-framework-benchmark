...
func (s *server) AggregateOrders(ctx context.Context, req *pb.AnalyticsRequest) (*pb.AggregateResult, error) {
    ...
    // Copy maps to return safer results, as state is recycled
...
func main() {
	...
	s := grpc.NewServer(
		grpc.MaxConcurrentStreams(256), // Match Tonic config
		grpc.InitialWindowSize(1*1024*1024),     // 1MB
		grpc.InitialConnWindowSize(10*1024*1024), // 10MB
	)
